...
- **RAM scales per service; renderer pooling is not possible.** Each service runs in its own Chromium renderer **and** its own `RequestContext` (separate storage partition, for per-service login isolation). Chromium keeps a separate renderer per partition, so `--renderer-process-limit` / disabling site isolation does **not** pool them (measured: ~5% at 8 services, vs ~46% for many tabs in one profile). To pool you'd have to share one context → share cookies → lose per-service logins. So the only RAM lever is the lifecycle (freeze/hibernate), controlled per service via the **Sleep** toggle — which trades a service's notifications for its RAM.
