...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // App-id (--class) and GPU-stability switches (software rendering by
    // default; --disable-features=Vulkan when OMNICHAT_ENABLE_GPU=1) are applied
    // in OmniChatApp::on_before_command_line_processing — command_line_get_global()
    // returns None this early (before initialize()), so switches set here never
    // take effect.
